...
во второй — целое число i(0≤i≤7) — вердикт интерактора, в третьей — целое число c(0≤c≤7) — вердикт чекера.
...
def realization() -> int:
    ...
    if interactor_verdict == 0:
        return 3 if (exit_code != 0) else checker_verdict

    if interactor_verdict == 1:
        return checker_verdict

    if interactor_verdict == 4:
        return 3 if exit_code != 0 else 4

    if interactor_verdict == 6:
        return 0

    if interactor_verdict == 7:
        return 1
